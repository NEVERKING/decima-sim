...
class ActorAgent(Agent):
    def __init__(self, sess, node_input_dim, job_input_dim, hid_dims, output_dim,
        ...
        self.sess = sess
        self.node_input_dim = node_input_dim  # node输入维度 5；在gnn中使用
        self.job_input_dim = job_input_dim  # job输入维度 3
        self.hid_dims = hid_dims  # 隐藏层维度 [16, 8]
        self.output_dim = output_dim  # embedding输出维度 8
        self.max_depth = max_depth  # 树的最大深度
        self.executor_levels = executor_levels  # 可选的 parallelism
        self.eps = eps
        self.act_fn = act_fn
    ...
    def translate_state(self, obs):
        ...
        # [整理得到 job的 features（3个特征）]  gather job level inputs
        job_idx = 0
        for job_dag in job_dags:
            # 1. number of executors in the job
            job_inputs[job_idx, 0] = exec_map[job_dag] / 20.0
            # 2. the current executor belongs to this job or not
            if job_dag is source_job:
                job_inputs[job_idx, 1] = 2
            else:
                job_inputs[job_idx, 1] = -2
            # 3. number of source executors
            job_inputs[job_idx, 2] = num_source_exec / 20.0

            job_idx += 1

        # [整理得到 node的 features（5个特征）] gather node level inputs
        node_idx = 0
        job_idx = 0
        for job_dag in job_dags:
            for node in job_dag.nodes:
                # 1. [先是job的] copy the feature from job_input first
                node_inputs[node_idx, :3] = job_inputs[job_idx, :3]

                # 2. 计算量 work on the node
                node_inputs[node_idx, 3] = \
                    (node.num_tasks - node.next_task_idx) * \
                    node.tasks[-1].duration / 100000.0

                # 3. 剩余task数量 number of tasks left
